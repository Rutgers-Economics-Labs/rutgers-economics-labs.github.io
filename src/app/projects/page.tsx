      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-8">
          {projects.map((project, idx) => (
            <div key={idx} className="bg-[var(--card-bg)] border border-[var(--card-border)] card-hover p-8 rounded-2xl">
              <div className="flex flex-col lg:flex-row items-start lg:items-center lg:space-x-6">
                ...
                <div className="flex-1 w-full">
                  <h4 className="text-2xl font-bold text-[var(--text-primary)] mb-3">{project.title}</h4>
                  ...
                  <div className={project.isPublic ? 'flex flex-wrap items-center gap-2 mb-4' : 'flex items-center space-x-4 mb-2'}>
                    {project.tags.map((tag, i) => (
                      <span
                        ...
                        className={
                          tag.color === 'green'
                            ? 'bg-green-600/20 text-green-500 px-3 py-1 rounded-full text-sm font-medium'
                            : tag.color === 'blue'
                              ? 'bg-blue-600/20 text-blue-500 px-3 py-1 rounded-full text-sm font-medium'
                              : tag.color === 'yellow'
                                ? 'bg-yellow-600/20 text-yellow-500 px-3 py-1 rounded-full text-sm font-medium'
                                : 'bg-[var(--bg-tertiary)] text-[var(--text-muted)] px-3 py-1 rounded-full text-xs font-semibold'
                        }
                      >
                        {tag.label}
                      </span>
